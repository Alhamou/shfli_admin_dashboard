...
export function ItemDetailView({ uuid, open, onClose, onStatusChange }: ItemDetailViewProps) {
  ...
  const handleStatusToggle = async (newStatus: 'active' | 'blocked') => {
    ...
    try {
      ...
      toast.success( `Item has been ${newStatus === 'active' ? 'enabled' : 'disabled'}`)
  ...
  <DialogContent className="max-w-[95vw] w-full max-h-[90vh] overflow-y-auto">
    ...
    {loading ? (
      ...
      <div className="text-center py-8 text-destructive">
        {error}
        <Button variant="outline" className="mt-4" onClick={fetchItem}>
          Retry
        </Button>
      </div>
    ) : item ? (
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant={item.is_active === 'active' ? 'default' : 'destructive'}>
                {item.is_active}
              </Badge>
              {item.delivery_available && <Badge variant="secondary">Delivery Available</Badge>}
              {item.installment && <Badge variant="secondary">Installment</Badge>}
          ...
          {item.item_as === 'job' ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{item.need ? 'Employee looking for a job' : 'Company looking for an employee'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium">Remote Job</p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {(item.item_as === 'shop' || item.item_as === 'used') && item.storage_capacity && (
                  <>
                    <div>
              ...
              <p>
                {item.price ? `${item.price.toLocaleString()} ${item.currency}` : 'N/A'}
                {item.discount > 0 && (
                ...
                <div>
                  ...
                  <p>{item.date_end_discount ? new Date(item.date_end_discount).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuItem onClick={() => setSelectedReason('Other')}>
                    Other
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button
                  ...
                  disabled={!selectedReason || (selectedReason === 'Other' && !customReason) || updatingStatus}
                >
                  Confirm Block
                </Button>
              </div>
