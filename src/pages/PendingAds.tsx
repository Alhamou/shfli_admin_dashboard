...
import { Briefcase, Clock, Eye, Hash, LayoutGrid, List, MapPin, MoreHorizontal, Search, Tag, X } from "lucide-react";
import "moment";
...
export default function PendingAds() {
  ...
  const getStatusBadge = (status: string) => {
    return (
      <CustomBadge variant={status as any} size="lg" className="px-3 py-1 font-bold shadow-sm">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-2xl font-black tracking-tight text-foreground">إعلانات قيد المراجعة</h1>
          </div>
          <p className="text-muted-foreground font-medium flex items-center gap-2 mr-13">
            ...
            يوجد حالياً {pagination.total} إعلان ينتظر الموافقة
          </p>
        </div>
      ...
      {/* Main Content Area */}
      <div
        className="h-[calc(100vh-200px)] overflow-y-auto pr-2 -mr-2 custom-scrollbar"
      >
        <div className="space-y-6 pb-20">
            ...
            {/* Desktop View: Interactive Table or Grid */}
                  ...
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow className="hover:bg-transparent border-border/40 h-16">
                        <TableHead className="w-[280px] font-black uppercase tracking-widest text-[10px] text-muted-foreground mr-1 text-right">تفاصيل الإعلان</TableHead>
            ...
            {!hasMore && items.length > 0 && (
              <div className="py-12 text-center animate-in fade-in duration-1000">
                <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-muted/30 border border-border/40">
                    ...
                    <p className="text-sm font-black text-muted-foreground">لا توجد المزيد من الإعلانات للمراجعة</p>
                    <div className="h-2 w-2 rounded-full bg-muted-foreground/30"></div>
                </div>
              </div>
            )}
            ...
            {!loading && items.length === 0 && (
              <div className="py-32 text-center flex flex-col items-center animate-in zoom-in-95 duration-500">
                <div className="h-24 w-24 rounded-full bg-emerald-500/10 flex items-center justify-center mb-6 shadow-inner text-emerald-500">
                   <Clock className="h-10 w-10 animate-pulse" />
                </div>
                <h3 className="text-2xl font-black text-foreground mb-3">السجل فارغ حالياً</h3>
                <p className="text-muted-foreground max-w-sm font-medium leading-relaxed">
                  تمت مراجعة جميع الإعلانات بنجاح. لا توجد إعلانات معلقة في الوقت الحالي.
                </p>
              </div>
            )}
