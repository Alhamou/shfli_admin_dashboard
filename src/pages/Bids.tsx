...
export default function BidsScreen() {
  ...
  const fetchItems = useCallback(
    async (
      ...
      try {
        const query = toQueryString({
          ...
          limit,
          item_for: "bid",
          bid_status: activeTab, // Add active tab value to the query
          ...(search !== ""
            ? isUUIDv4(search)
              ? { uuid: search }
              : { id: search }
    ...
    [loading, activeTab] // Add activeTab to dependencies
  );
  ...
  // Refetch items when tab changes
  useEffect(() => {
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            <TabsList className="grid w-full grid-cols-5">
              <TabsTrigger value="pending">
                {t("dashboard.bidStatus.pending")}
              </TabsTrigger>
              <TabsTrigger value="ended">
                {t("dashboard.bidStatus.ended")}
              </TabsTrigger>
              <TabsTrigger value="active">
                {t("dashboard.bidStatus.active")}
              </TabsTrigger>
              <TabsTrigger value="blocked">
                {t("dashboard.bidStatus.blocked")}
              </TabsTrigger>
              <TabsTrigger value="ask_edit">
                {t("dashboard.bidStatus.ask_edit")}
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
